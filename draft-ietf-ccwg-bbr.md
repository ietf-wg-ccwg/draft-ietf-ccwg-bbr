...
access to the (RS) rate sample populated while processing that ACK.
...
C.cwnd: The transport sender's congestion window. When transmitting data,
...
fully utilized its available bandwidth since it most recently started looking.
...
was obtained, or at which the most recent ProbeRTT completed and revalidated
the estimate.
...
BBR.ProbeRTTDuration: A constant specifying the minimum duration for which
ProbeRTT holds C.inflight at or below BBR.probe_rtt_cwnd: 200 ms.

BBR.ProbeRTTInterval: A constant specifying the minimum time interval between
ProbeRTT states: 10 secs. This is equal to BBR.MinRTTFilterLen.

BBR.probe_rtt_cwnd: The C.cwnd cap used during ProbeRTT. Upon entering
ProbeRTT, BBR calculates this value from the current BBR.bw and the
pre-expiration BBR.min_rtt, and holds it constant throughout the ProbeRTT
episode.
...
Whenever possible, TCP connections using BBR as a congestion controller SHOULD
...
#### Accelerating to Probe for Bandwidth  {#bandwidth-probing-accel}
...
* Precautionary Bandwidth Probing: Acceleration: If a round trip elapses
  ...
  C.inflight at BBR.inflight_longterm without precautionary deceleration.

#### Exit Conditions for Bandwidth Probing  {#exit-conditions-for-bandwidth-probing-}

The BBR flow ends ProbeBW_UP bandwidth probing and
...
3. Precautionary Bandwidth Probing: Deceleration: If the previous
...
every BBR.ProbeRTTInterval = 10 seconds, the flow enters ProbeRTT, decelerating
...
BBR uses the same 10-second interval for BBR.MinRTTFilterLen and
BBR.ProbeRTTInterval. When this interval expires, BBR calculates and saves the
ProbeRTT cwnd cap using the previous BBR.min_rtt before replacing the expired
BBR.min_rtt with a new sample. This provides a robust BDP estimate for the
C.cwnd = 0.5\*BDP calculation even if the RTT sample that triggers ProbeRTT is
inflated by queuing. BBR holds this cwnd cap constant throughout the ProbeRTT
episode, increasing the likelihood of fully draining the bottleneck queue.

The algorithm for ProbeRTT is as follows:

Entry conditions: In any state other than ProbeRTT itself, if the BBR.min_rtt
estimate has not been updated or revalidated for more than
BBR.ProbeRTTInterval = 10 seconds, then BBR enters ProbeRTT and reduces the
BBR.cwnd_gain to BBR.ProbeRTTCwndGain = 0.5.
...
BBR uses a BBR.MinRTTFilterLen equal to BBR.ProbeRTTInterval, so that expiration
of the BBR.min_rtt estimate also schedules the next ProbeRTT.

To allow coordination with other BBR flows, each BBR flow MUST use the
standard BBR.ProbeRTTInterval of 10 secs.

A BBR.ProbeRTTInterval of 10 secs is short enough to allow timely convergence
if traffic levels or routes change, but long enough so that interactive
...
is low enough for long enough to drain its queue in the bottleneck. Such a
period can produce a lower RTT sample that refreshes BBR.min_rtt without
requiring ProbeRTT. This way, flows typically need only pay the 1 percent
throughput penalty if there are multiple bulk flows busy sending over the
entire BBR.ProbeRTTInterval window.

As an optimization, when restarting from idle and finding that the
BBR.min_rtt estimate has expired, BBR does not enter ProbeRTT; the idleness is
deemed a sufficient attempt to coordinate to drain the queue.
...
handle the steps related to the ProbeRTT state. If BBR.min_rtt has expired,
BBR first enters ProbeRTT and calculates BBR.probe_rtt_cwnd using the old,
pre-expiration BBR.min_rtt. BBR then updates BBR.min_rtt using the latest RTT
sample. Thus, an inflated RTT sample that triggers ProbeRTT cannot inflate the
cwnd cap used to drain the bottleneck queue.
...
~~~~
  ...
  EnterProbeRTT():
    BBR.probe_rtt_cwnd =
      max(BDPMultiple(BBR.ProbeRTTCwndGain), BBR.MinPipeCwnd)
    BBR.state = ProbeRTT
    BBR.pacing_gain = 1
...
* The BBR.MinRTTFilterLen is equal to BBR.ProbeRTTInterval, so expiration of
  BBR.min_rtt schedules a ProbeRTT (see the "ProbeRTT" section below). Before
  replacing an expired BBR.min_rtt, BBR saves a ProbeRTT cwnd cap based on the
  previous estimate. This helps ProbeRTT reduce C.inflight below the estimated
  BDP even if the RTT sample that triggers ProbeRTT is inflated by queuing.
...
send quantum, and congestion window.
...
### Pacing Rate: C.pacing_rate {#pacing-rate-bbrpacingrate}
...
### Send Quantum: C.send_quantum {#send-quantum-bbrsendquantum}
...
A BBR implementation MAY use alternate approaches to select a C.send_quantum,
...
By default, BBR grows C.cwnd to meet its BBR.max_inflight, which models
...
implementation's initial congestion window (e.g. from {{RFC6928}} for TCP).
...
For BBR.max_inflight, BBR imposes a floor of BBR.MinPipeCwnd (4 packets, i.e.
...
Upon a retransmission timeout (RTO), BBR conservatively reduces C.cwnd to a
...
When BBR exits loss recovery it restores C.cwnd to the "last known good"
...
The high-level design for updating C.cwnd in loss recovery is as follows:
...
the bottleneck queue, thereby allowing measurement of BBR.min_rtt. Upon entry,
BBR calculates BBR.probe_rtt_cwnd from the current BBR.bw and the
pre-expiration BBR.min_rtt. Throughout the ProbeRTT episode, BBR bounds C.cwnd
to this saved value, which has a floor of BBR.MinPipeCwnd to allow pipelining
...
Finally, BBR bounds C.cwnd based on recent congestion, as outlined in the
...
In order to cover the wide set of real-world use cases, test cases can be
run with a variety of RTTs (e.g., < 1 ms to 1 s) and bandwidths (e.g., 64 kbit/s to 10 Gbit/s).
...
data so that it exits STARTUP and naturally gets to the specified phase.
...
4. Verify that after 3 consecutive rounds where the delivery rate grows by less
   than 25% (i.e., when BBR.full_bw_count reaches 3), `BBR.full_bw_now` and `BBR.full_bw_reached` are set to `true`.
5. Verify the connection transitions from STARTUP to DRAIN.

## Exiting STARTUP on Loss When Application-Limited

1. Start a flow in the STARTUP phase.
2. Limit the application sending rate so bandwidth samples are application-limited.
3. Introduce packet loss exceeding `BBR.LossThresh` (2%) and sequence range
...
5. Verify the connection sets `BBR.full_bw_now` and `BBR.full_bw_reached` to `true`, exits STARTUP,
...
## Exiting DRAIN Based on Inflight
...
3. Ensure `C.inflight` remains higher than `BBRInflight(1.0)` for several
   round trips.
4. Track `BBR.drain_start_round` and current `BBR.round_count`.
5. Verify that after 3 round trips in DRAIN, the connection exits DRAIN and
   transitions to PROBE_BW even though `C.inflight` has not reached the target
   BDP.

## Exiting PROBE_UP on Bandwidth Plateau
...
4. Verify that `BBRIsTimeToGoDown()` triggers when `BBR.full_bw_now` is set to
   `true` after 3 consecutive rounds where the delivery rate grows by less than 25%
   (i.e., when BBR.full_bw_count reaches 3).
5. Verify the connection transitions to PROBE_DOWN.

## Exiting PROBE_UP on Loss When Application-Limited
...
3. Verify that after `BBR.bw_probe_wait` time elapses, `BBRIsTimeToProbeBW()`
   returns `true`.
4. Verify the connection transitions directly to PROBE_REFILL, bypassing
   PROBE_CRUISE.

## Entering and Exiting PROBE_RTT

1. Wait until the connection transitions to PROBE_BW.
2. Record `BBR.bw` and `BBR.min_rtt`, and calculate the expected
   `BBR.probe_rtt_cwnd` as the maximum of `BBR.ProbeRTTCwndGain * BBR.bw *
   BBR.min_rtt` and `BBR.MinPipeCwnd`.
3. Wait for `BBR.ProbeRTTInterval` (10 seconds) without updating
   `BBR.min_rtt`, and provide an inflated RTT sample when the interval expires.
4. Verify the connection enters PROBE_RTT, sets `BBR.cwnd_gain` to
   `BBR.ProbeRTTCwndGain` (0.5), and sets `BBR.probe_rtt_cwnd` to the expected
   value calculated from the pre-expiration BBR.min_rtt.
...
## Achieving Expected STARTUP Bandwidth on a Link With Aggregation
...
3. Verify that `BBR.extra_acked` is calculated correctly from the bursty ACK
   arrivals and that `C.cwnd` > 2 * BDP.
...
## Achieving Expected Cruise Bandwidth on a Link With Aggregation
...
3. Verify that `BBR.extra_acked` updates correctly using the max filter over the
   last `BBR.ExtraAckedFilterLen` round trips.
...
## Correctly Managing Sub-Packet BDPs
...
## Increasing Bandwidth 10x and Ensuring Full Bandwidth is Reached

1. Wait until the connection transitions to PROBE_BW.
2. Suddenly increase the bottleneck bandwidth by 10x (e.g., from 10 Mbit/s to 100 Mbit/s).
3. Verify that during PROBE_UP, the step size added to `BBR.inflight_longterm`
   doubles each round trip (resulting in exponential growth).
4. Ensure the connection discovers the full 100 Mbit/s bandwidth within a
   reasonable number of round trips (O(log(BDP))).

## Decreasing Bandwidth 10x and Ensuring Max Bandwidth Adapts Down

1. Wait until the connection transitions to PROBE_BW.
2. Suddenly decrease the bottleneck bandwidth by 10x (e.g., from 100 Mbit/s to 10 Mbit/s).
3. Verify that `BBR.bw_shortterm` rapidly adapts downwards due to the resulting
   packet loss.
4. Verify that after the `BBR.max_bw` window expires (2 PROBE_BW cycles, or `BBR.MaxBwFilterLen`),
   `BBR.max_bw` drops to the new 10 Mbit/s limit, matching the path's delivery
   rate.

## Handling Token Bucket Policers
...
## Handling Spurious Fast Recovery
...
2. Verify `BBRSaveStateUponLoss()` saves the current `BBR.state`,
   `BBR.bw_shortterm`, `BBR.inflight_shortterm` (to `BBR.undo_inflight_shortterm`), and `BBR.inflight_longterm` (to `BBR.undo_inflight_longterm`).
3. Once the transport protocol detects the loss was spurious (e.g., via TCP DSACK or QUIC packet numbers indicating the original packet was delivered and the retransmission was spurious), verify
...
## Handling Spurious RTO Recovery

1. Introduce packet reordering on the network path that triggers a spurious RTO
   Recovery.
2. Verify `BBRSaveStateUponLoss()` saves the current `BBR.state`,
   `BBR.bw_shortterm`, `BBR.inflight_shortterm` (to `BBR.undo_inflight_shortterm`), and `BBR.inflight_longterm` (to `BBR.undo_inflight_longterm`).
3. Once the transport protocol detects the loss was spurious (e.g., via TCP DSACK or QUIC packet numbers indicating the original packet was delivered and the retransmission was spurious), verify
...
1. Start a flow and artificially trigger PROBE_RTT before `BBR.full_bw_reached`
   becomes `true` (e.g., by ensuring `BBR.ProbeRTTInterval` expires during an
...
## Handling Loss During PROBE_UP After inflight_longterm is Set
...
## Handling Application-Limited Sending During PROBE_REFILL
...
3. Verify that because the bandwidth samples during this round are marked as
   application-limited and are lower than `BBR.max_bw` (`RS.delivery_rate` < `BBR.max_bw`), BBR does not update its max bandwidth
   estimate with these artificially low values.
